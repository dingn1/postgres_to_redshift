...
class PgToRedshift
  ...
  def self.update_tables
    update_tables = PgToRedshift.new
  ...
  def self.target_connection
    @target_connection = PG::Connection.new(:host => target_uri.host, :port => target_uri.port, :user => target_uri.user || ENV["USER"], :password => target_uri.password, :dbname => target_uri.path[1..-1]) unless instance_variable_defined?(:"@target_connection")
  ...
  def s3
    ...
    @s3 ||= Aws::S3::Resource.new.bucket(ENV["S3_DATABASE_EXPORT_BUCKET"])
  end
  ...
  def copy_table(table)
    ...
    chunk = 1
    s3.objects.with_prefix("export/#{table.target_table_name}.psv.gz").delete_all
    begin
  ...
  def upload_table(table, buffer, chunk)
    ...
    s3.objects["export/#{table.target_table_name}.psv.gz.#{chunk}"].write(buffer, :acl => :authenticated_read)
  end
