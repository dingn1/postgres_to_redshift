...
RSpec.describe "a small source database", :type => :feature do
  context "with a simple column" do
    before(:all) do
      PgToRedshift::Test.test_connection.exec(<<-EOS,
    ...
    after(:all) do
      PgToRedshift::Test.test_connection.exec('DROP TABLE IF EXISTS "p2r_integration";')
      PgToRedshift::Test.test_target_connection.exec('DROP TABLE IF EXISTS "p2r_integration";')
    end

    it "Copies all rows to target table" do
      PgToRedshift.update_tables
      result = PgToRedshift::Test.test_target_connection.exec(
