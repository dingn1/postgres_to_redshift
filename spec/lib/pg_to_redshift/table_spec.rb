...
RSpec.describe PgToRedshift::Table do
  context "with a simple table" do
    before do
      ...
      @table = PgToRedshift::Table.new(:attributes => attributes, :columns => columns)
    end
  ...
  describe "#view?" do
    it "returns true if it is a view" do
      ...
      table = PgToRedshift::Table.new(:attributes => attributes)
